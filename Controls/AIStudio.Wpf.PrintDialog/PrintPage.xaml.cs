...
using AIStudio.Wpf.PrintDialog.Effect;
using AIStudio.Wpf.PrintDialog.Helpers;
using AIStudio.Wpf.PrintDialog.PrintControl;

namespace AIStudio.Wpf.PrintDialog
{
    /// <summary>
    ...
    partial class PrintPage : Page
    {
        ...
        private readonly PrintDialogSettings _defaultSettings;
        private readonly System.Windows.Controls.PrintDialog _systemPrintDialog;
        private readonly Func<DocumentInfo, List<PageContent>> _getDocumentWhenReloadDocumentMethod;
        ...
        public PrintPage(FixedDocument document, string documentName, double pageMargin, PrintDialogSettings defaultSettings, bool allowPagesOption, bool allowScaleOption, bool allowTwoSidedOption, bool allowPagesPerSheetOption, bool allowPageOrderOption, bool allowMoreSettingsExpander, bool allowAddNewPrinerComboBoxItem, bool allowPrinterPreferencesButton, Func<DocumentInfo, List<PageContent>> getDocumentWhenReloadDocumentMethod)
        {
            isLoaded = false;
            ...
            try
            {
                ...
                foreach (PrintQueue printer in _localDefaultPrintServer.GetPrintQueues())
                {
                    ...
                    Image printerIcon = new Image()
                    {
                        Width = 55,
                        Height = 55,
                        Source = PrinterIconHelper.GetPrinterIcon(printer, _localDefaultPrintServer),
                        Stretch = Stretch.Fill
                    };
                ...
                equipmentComboBox.Tag = PrinterIconHelper.GetPrinterIcon(_localDefaultPrintServer.GetPrintQueue((equipmentComboBox.SelectedItem as ComboBoxItem).Tag.ToString()), _localDefaultPrintServer, true);
        ...
        private void ReloadDocument()
        {
            if (isRefresh == true)
            {
                ...
                try
                {
                    ...
                    {
                        ScrollViewer scrollViewer = (ScrollViewer)documentPreviewer.Template.FindName("PART_ContentHost", documentPreviewer);
                        scrollViewer.Effect = new GrayscaleEffect();
                    ...
                    {
                        pageContentList = _getDocumentWhenReloadDocumentMethod(new DocumentInfo()
                        {
                            Color = color,
                            Margin = margin,
                            Orientation = (PrintSettings.PageOrientation)orientationComboBox.SelectedIndex,
                            PageOrder = (Helpers.PageOrder)pageOrderComboBox.SelectedIndex,
                    ...
                    if (pagesPerSheetComboBox.SelectedIndex != 0)
                    {
                        MultiPagesPerSheetHelper multiPagesPerSheetHelper = new MultiPagesPerSheetHelper(int.Parse((pagesPerSheetComboBox.SelectedItem as ComboBoxItem).Content.ToString()), doc, _orginalPageSize, (DocumentOrientation)orientationComboBox.SelectedIndex, (Helpers.PageOrder)pageOrderComboBox.SelectedIndex);
        ...
        private void Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            pagesPerSheetComboBox.SelectedIndex = MultiPagesPerSheetHelper.GetPagePerSheetCountIndex(_defaultSettings.PagesPerSheet);
