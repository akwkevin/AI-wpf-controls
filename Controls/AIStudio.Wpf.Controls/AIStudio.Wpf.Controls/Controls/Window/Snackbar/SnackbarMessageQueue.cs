...
namespace AIStudio.Wpf.Controls
{
    public class SnackbarMessageQueue : ISnackbarMessageQueue, IDisposable
    {
        ...
        public SnackbarMessageQueue() : this(TimeSpan.FromSeconds(1))
        {
        }
        ...
        public void SetDuration(TimeSpan messageDuration)
        {
            _messageDuration = messageDuration;
        }

        //插入在队尾
        public void Enqueue(object content, ControlStatus controlStatus, VerticalAlignment verticalAlignment)
        {
            Enqueue(content, false, controlStatus, verticalAlignment);
        }

        //插入到队头
        public void Enqueue(object content, bool promote, bool neverConsiderToBeDuplicate, ControlStatus controlStatus, VerticalAlignment verticalAlignment)
        {
            Enqueue(content, null, null, null, promote, neverConsiderToBeDuplicate, controlStatus, verticalAlignment);
        }

        public void Enqueue(object content, bool neverConsiderToBeDuplicate, ControlStatus controlStatus, VerticalAlignment verticalAlignment)
        {
            if (content == null) throw new ArgumentNullException(nameof(content));

            Enqueue(content, null, null, null, false, neverConsiderToBeDuplicate, controlStatus, verticalAlignment);
        }

        public void Enqueue(object content, object actionContent, Action actionHandler, ControlStatus controlStatus, VerticalAlignment verticalAlignment)
        {
            Enqueue(content, actionContent, actionHandler, false, controlStatus, verticalAlignment);
        }

        public void Enqueue(object content, object actionContent, Action actionHandler, bool promote, ControlStatus controlStatus, VerticalAlignment verticalAlignment)
        {
            if (content == null) throw new ArgumentNullException(nameof(content));
            ...
            Enqueue(content, actionContent, _ => actionHandler(), promote, false, false, controlStatus, verticalAlignment);
        }

        public void Enqueue<TArgument>(object content, object actionContent, Action<TArgument> actionHandler,
            TArgument actionArgument, ControlStatus controlStatus, VerticalAlignment verticalAlignment)
        {
            Enqueue(content, actionContent, actionHandler, actionArgument, false, false, controlStatus, verticalAlignment);
        }

        public void Enqueue<TArgument>(object content, object actionContent, Action<TArgument> actionHandler,
            TArgument actionArgument, bool promote, ControlStatus controlStatus, VerticalAlignment verticalAlignment) =>
            Enqueue(content, actionContent, actionHandler, actionArgument, promote, promote, controlStatus, verticalAlignment);
            ...
            TArgument actionArgument, bool promote, bool neverConsiderToBeDuplicate, ControlStatus controlStatus, VerticalAlignment verticalAlignment)
        {
            ...
            Action<object> handler = actionHandler != null
                ...
                : null;
            Enqueue(content, actionContent, handler, actionArgument, promote, neverConsiderToBeDuplicate, controlStatus, verticalAlignment);
        }

        public void Enqueue(object content, object actionContent, Action<object> actionHandler,
            object actionArgument, bool promote, bool neverConsiderToBeDuplicate, ControlStatus controlStatus, VerticalAlignment verticalAlignment)
        {
            if (content == null) throw new ArgumentNullException(nameof(content));
            ...
            var snackbarMessageQueueItem = new SnackbarMessageQueueItem(content, actionContent, actionHandler,
                actionArgument, promote, neverConsiderToBeDuplicate, controlStatus, verticalAlignment);
            if (promote)
            ...
            if (Interlocked.Increment(ref _isPump) == 1)
            {
                Task.Factory.StartNew(PumpAsync);
            }
            else
            {
                Interlocked.Decrement(ref _isPump);
            }
        }
        ...
        private DispatcherOperation<Snackbar> FindSnackbar(Dispatcher dispatcher)
        {
            return dispatcher.InvokeAsync(() => {
                return _pairedSnackbars.FirstOrDefault(sb => {
                    ...
                    var window = System.Windows.Window.GetWindow(sb);
            ...
            SnackbarMessageQueueItem messageQueueItem, EventWaitHandle actionClickWaitHandle)
        {
            ...
            snackbar.SetCurrentValue(FrameworkElement.VerticalAlignmentProperty, messageQueueItem.VerticalAlignment);
            snackbar.SetCurrentValue(ControlAttach.StatusProperty, messageQueueItem.ControlStatus);
