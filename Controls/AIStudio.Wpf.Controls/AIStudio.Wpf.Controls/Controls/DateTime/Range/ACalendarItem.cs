...
namespace AIStudio.Wpf.Controls
{
    ...
    public class ACalendarItem : Control
    {
        #region 枚举
        ...
        #region Fields
        private ACalendarDayButton[,] CalendarDayButtons = new ACalendarDayButton[7, 7];
        private ACalendarButton[,] CalendarButtons = new ACalendarButton[3, 4];
        #endregion
        ...
        #region Constructors
        static ACalendarItem()
        {
            DefaultStyleKeyProperty.OverrideMetadata(typeof(ACalendarItem), new FrameworkPropertyMetadata(typeof(ACalendarItem)));
        }
        #endregion
        ...
        private void InitMonthGrid()
        {
            ...
            for (int i = 0; i < 7; i++)
            {
                ACalendarDayButton calendarDayButton = new ACalendarDayButton();
        ...
        private void InitYearGrid()
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < 4; j++)
                {
                    ACalendarButton calendarButton = new ACalendarButton();
                    calendarButton.SetValue(Grid.RowProperty, i);
                    calendarButton.SetValue(Grid.ColumnProperty, j);
                    calendarButton.SetValue(ACalendarButton.HasSelectedDatesProperty, false);
                    calendarButton.SetBinding(FrameworkElement.StyleProperty, new Binding("CalendarButtonStyle") { Source = this.Owner });
        ...
        {
            ACalendarDayButton calendarDayButton = sender as ACalendarDayButton;
        ...
        {
            ACalendarButton calendarButton = sender as ACalendarButton;
        ...
        private void SetMonthModeCalendarDayButtons()
        {
            ...
            //获取该月第一天所在的列数
            int firstColIndex = (firstDay.DayOfWeek - this.Owner.FirstDayOfWeek + 7) % 7;
            ...
            this.SetSelectedDatesHighlight(this.Owner.SelectedDates);
        ...
        public void SetSelectedDatesHighlight(ObservableCollection<DateTime> selectedDates)
        {
            ...
            {
                ACalendarDayButton dayButton = item as ACalendarDayButton;
                ...
                {
                    if (dt == selectedDates.FirstOrDefault() || dt == selectedDates.LastOrDefault())
                    {
            ...
            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < 4; j++)
                {
                    ...
                    if (this.Owner != null && this.Owner.DisplayDate != null
                        && MonthIsEqual(dateTime, this.Owner.DisplayDate))
                    {
        ...
        private void SetYearButtons()
        {
            ...
            foreach (object item in this.PART_YearView.Children)
            {
                ...
                ACalendarButton calendarButton = item as ACalendarButton;
